...
import { flatten, flow, get, isPlainObject, mapValues, omitByUndefined, spreadKeys } from './utils';
...
const applyMapper = (mapper) => (source) => {
  if (typeof mapper === 'string') {
    ...
    return flatten(
      mapper.map((nestedMapper) => applyMapper(nestedMapper)(source))
    );
  }
  throw new TypeError(`cannot apply mapper '${mapper}', need array|function|object|string, is ${typeof mapper}`);
};
